...
describe('Transaction Routes E2E Tests', () => {
    ...
    it('POST /api/transactions should return 201 when creating a transaction sucessfully', async () => {
    ...
    it('PATCH /api/transactions/:transactionId should return 200 when updating a transaction sucessfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({
                ...user,
    ...
    it('PATCH /api/transactions/:transactionId should return 404 when updating a non-existing transaction', async () => {
        ...
        const response = await request(app)
            .patch(`/api/transacitons/${transaction.id}`)
            .set('Authorization', `Bearer ${createdUser.tokens.accessToken}`)
            .send({ amount: 100, type: TransactionType.INVESTMENT })
    ...
    it('DELETE /api/transactions/:transactionId should return 404 when deleting a non-existing transaction', async () => {
        ...
        const response = await request(app)
            .delete(`/api/transacitons/${transaction.id}`)
            .set('Authorization', `Bearer ${createdUser.tokens.accessToken}`)
