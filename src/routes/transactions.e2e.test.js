...
describe('Transaction Routes E2E Tests', () => {
    ...
    it('POST /api/transactions/me should return 201 when creating a transaction sucessfully', async () => {
    ...
    it('GET /api/transactions?userId should return 200 when fetching transactions sucessfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({
                ...user,
    ...
    it('PATCH /api/transactions/me/:transactionId should return 200 when updating a transaction sucessfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({
                ...user,
    ...
    it('DELETE /api/transactions/:transactionId should return 200 when deleting a transaction sucessfully', async () => {
        const { body: createdUser } = await request(app)
            .post('/api/users')
            .send({
                ...user,
    ...
    it('PATCH /api/transactions/:transactionId should return 404 when updating a non-existing transaction', async () => {
        ...
        const response = await request(app)
            .patch(`/api/transacitons/me/${transaction.id}`)
            .set('Authorization', `Bearer ${createdUser.tokens.accessToken}`)
            .send({ amount: 100, type: TransactionType.INVESTMENT })
    ...
    it('DELETE /api/transactions/:transactionId should return 404 when deleting a non-existing transaction', async () => {
        ...
        const response = await request(app)
            .delete(`/api/transacitons/me/${transaction.id}`)
            .set('Authorization', `Bearer ${createdUser.tokens.accessToken}`)
